...
class DeepFashionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        ret = {}
        # ret['image_name'] = self.root + sample['image_name']
        # ret['bbox'] = np.array([sample.x_1, sample.y_1, sample.x_2, sample.y_2])
        ret['image_np'] = img
        ret['image'] = image
...
class FLDDataset(torch.utils.data.Dataset):
    ...
    def plot_sample(self, i):
        sample = self[i]
        sample_name = self.ds.iloc[i]
        print(sample_name['image_name'])
